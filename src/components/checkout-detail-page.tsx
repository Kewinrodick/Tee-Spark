...
export function CheckoutDetailPageClient({ initialDesign, id }: { initialDesign: Design | undefined, id: string }) {
    const [design, setDesign] = useState<Design | null | undefined>(initialDesign);

    useEffect(() => {
        // If the design is not passed from the server, or for client-side navigation,
        // we fetch it on the client.
        if (!design) {
            const storedDesigns = JSON.parse(localStorage.getItem('userDesigns') || '[]');
            const foundDesign = storedDesigns.find((d: Design) => d.id === id);
            setDesign(foundDesign || null); // Use null to indicate "not found"
        }
    }, [id, design]);
